...
pub fn resolve_type_reference(
    ...
    match (type_ref, context) {
        ...
        (insp::TypeRef::DefBody(body), _) => Ok(TypeRef::Anonymous(ontol_node_children(
            body.view().children(),
        ))
        .set_span(span)
        .into()),
        (insp::TypeRef::NumberRange(range), _) => {
            let start = range.start().stx_err()?;
            let end = range.end().stx_err()?;

            Ok(
                TypeRef::NumberRange(start.slice().into(), end.slice().into())
                    .set_span(span)
                    .into(),
            )
        }
...
pub fn lookup_type_path(
    ...
    while let Some(next) = symbols.next() {
        ...
        match &prev {
            ...
            SymEntryRef::LocalDef(_) | SymEntryRef::EdgeSymbol(..) | SymEntryRef::OntolDef(_) => {
                ...
                return Err(SemError::SubScoping);
            }
    ...
    match entry {
        ...
        Some(SymEntryRef::OntolModule(_)) => {
