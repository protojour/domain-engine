...
impl<'a, 'm> Unifier<'a, 'm> {
    ...
    fn unify_scoping2(
        ...
        match kind {
            ...
            NodeKind::Call(proc, args) => self.unify_call_scope((*proc, args), meta, u_node),
    ...
    fn unify_call_scope(
        &mut self,
        scope_call: (BuiltinProc, &[TypedHirNode<'m>]),
        scope_meta: Meta<'m>,
        u_node: UNode<'m>,
    ) -> UnifierResult<Unified2<'m>> {
        match u_node.kind {
            ...
            _ => self.unify_u_node2(u_node, None),
        }
    }

    fn invert_call(
        &mut self,
        (scope_proc, scope_args): (BuiltinProc, &[TypedHirNode<'m>]),
        scope_meta: Meta<'m>,
        subscope_idx: usize,
        u_node: UNode<'m>,
        ...
        match &pivot_arg.kind {
            NodeKind::VariableRef(var) => {
                ...
                let body = self.unify2(u_node, Some(&scope_args[subscope_idx]))?;
